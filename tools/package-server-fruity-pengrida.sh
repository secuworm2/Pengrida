...
mkdir -p "$bindir/"
cp "$executable" "$bindir/pengrida-server"
chmod 755 "$bindir/pengrida-server"
...
(
  ...
  echo "<dict>"
  echo "	<key>Label</key>"
  echo "	<string>re.pengrida.server</string>"
  echo "	<key>Program</key>"
  echo "	<string>$sysroot/usr/sbin/pengrida-server</string>"
  ...
  echo "</dict>"
  echo "</plist>"
) > "$daedir/re.pengrida.server.plist"
chmod 644 "$daedir/re.pengrida.server.plist"

installed_size=$(du -sk "$tmpdir" | cut -f1)

mkdir -p "$tmpdir/DEBIAN/"
cat >"$tmpdir/DEBIAN/control" <<EOF
Package: re.pengrida.server
Name: Pengrida
...
Section: Development
Conflicts: re.pengrida.server64
EOF
...
launchcfg=$sysroot/Library/LaunchDaemons/re.pengrida.server.plist
launchlog=\$(mktemp)
...
if [ "\$1" = remove ] || [ "\$1" = purge ]; then
  launchctl unload $sysroot/Library/LaunchDaemons/re.pengrida.server.plist &> /dev/null
fi
